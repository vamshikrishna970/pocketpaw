<!-- +layout.svelte — Agent OS layout: full-viewport wallpaper with gradient overlays.
     Updated: 2026-03-22 — Added liquid-glass CSS system from Interacly AI OS.
     Provides .liquid-glass utility class for all OS components.
     Theme vars: --c-glass, --c-light, --c-dark, --glass-reflex-*, --saturation.
     Hidden SVG filter for displacement-based glass distortion effect. -->
<script lang="ts">
  import "../../styles/global.css";
  import type { Snippet } from "svelte";
  import { onMount } from "svelte";

  let { children }: { children: Snippet } = $props();

  const WALLPAPER_URL =
    "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=2560&h=1440&fit=crop&crop=entropy";

  let wallpaperLoaded = $state(false);

  onMount(() => {
    const img = new Image();
    img.onload = () => { wallpaperLoaded = true; };
    img.onerror = () => { wallpaperLoaded = true; };
    img.src = WALLPAPER_URL;

    // Set glass theme variables on :root — dark theme for wallpaper-based OS
    const root = document.documentElement;
    root.style.setProperty("--c-glass", "#000000");
    root.style.setProperty("--c-light", "#fff");
    root.style.setProperty("--c-dark", "#000");
    root.style.setProperty("--glass-reflex-dark", "1");
    root.style.setProperty("--glass-reflex-light", "1");
    root.style.setProperty("--saturation", "150%");

    // Load saved glass settings
    let savedGlass = 38;
    let glassOn = true;
    try {
      const g = localStorage.getItem("paw-os-glass-opacity");
      if (g) savedGlass = parseInt(g, 10);
      const e = localStorage.getItem("paw-os-glass-enabled");
      if (e !== null) glassOn = e !== "false";
    } catch {}
    const glassStyle = document.createElement("style");
    glassStyle.id = "glass-opacity-style";
    if (glassOn) {
      document.body.classList.remove("glass-off");
      glassStyle.textContent = `.liquid-glass { background-color: color-mix(in srgb, #262621 ${savedGlass}%, transparent) !important; backdrop-filter: blur(8px) saturate(var(--saturation)) url(#switcher) !important; }`;
    } else {
      document.body.classList.add("glass-off");
      glassStyle.textContent = `.glass-off .liquid-glass { background-color: #262621 !important; backdrop-filter: none !important; border-color: rgba(255,255,255,0.10) !important; }`;
    }
    document.head.appendChild(glassStyle);

    document.body.style.overflow = "hidden";
    document.body.style.margin = "0";
    return () => {
      document.body.style.overflow = "";
    };
  });
</script>

<div class="os-viewport">
  <!-- Wallpaper image -->
  {#if wallpaperLoaded}
    <div
      class="wallpaper"
      style="background-image: url('{WALLPAPER_URL}');"
    ></div>
  {/if}

  <!-- Gradient overlays for text readability -->
  <div class="overlay overlay-diagonal"></div>
  <div class="overlay overlay-vertical"></div>

  <!-- Fallback gradient (visible while loading or on error) -->
  {#if !wallpaperLoaded}
    <div class="fallback"></div>
  {/if}

  <!-- OS content -->
  <div class="os-content">
    {@render children()}
  </div>
</div>

<!-- Hidden SVG filters for liquid glass effects -->
<svg style="position: fixed; width: 0; height: 0; pointer-events: none;" aria-hidden="true">
  <!-- Fractal noise filter for extra glassy texture on panels -->
  <filter id="glass-noise">
    <feTurbulence type="fractalNoise" baseFrequency="0.65" numOctaves="3" stitchTiles="stitch" result="noise" />
    <feColorMatrix type="saturate" values="0" in="noise" result="grayNoise" />
    <feBlend in="SourceGraphic" in2="grayNoise" mode="overlay" result="blended" />
    <feComponentTransfer in="blended">
      <feFuncA type="linear" slope="1" />
    </feComponentTransfer>
  </filter>

  <!-- Displacement filter for liquid glass distortion -->
  <filter id="switcher" primitiveUnits="objectBoundingBox">
    <feImage
      result="map"
      width="100%"
      height="100%"
      x="0"
      y="0"
      href="data:image/webp;base64,"
    />
    <feGaussianBlur in="SourceGraphic" stdDeviation="0.04" result="blur" />
    <feDisplacementMap in="blur" in2="map" scale="0.5" xChannelSelector="R" yChannelSelector="G" />
  </filter>
</svg>

<style>
  .os-viewport {
    position: fixed;
    inset: 0;
    width: 100vw;
    height: 100vh;
    overflow: hidden;
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif;
    color: rgba(255, 255, 255, 0.92);
    font-size: 13px;
    -webkit-font-smoothing: antialiased;
  }

  .wallpaper {
    position: absolute;
    inset: 0;
    z-index: 0;
    background-size: cover;
    background-position: center;
    background-repeat: no-repeat;
    animation: wallpaper-in 1s ease both;
  }

  @keyframes wallpaper-in {
    from { opacity: 0; filter: blur(10px); }
    to { opacity: 1; filter: blur(0); }
  }

  .overlay-diagonal {
    background: linear-gradient(135deg,
      rgba(0,0,0,0.30) 0%,
      rgba(0,0,0,0.15) 30%,
      rgba(0,0,0,0.10) 50%,
      rgba(0,0,0,0.20) 70%,
      rgba(0,0,0,0.40) 100%
    );
  }

  .overlay-vertical {
    background: linear-gradient(to bottom,
      rgba(0,0,0,0.35) 0%,
      rgba(0,0,0,0.05) 15%,
      transparent 40%,
      transparent 65%,
      rgba(0,0,0,0.15) 85%,
      rgba(0,0,0,0.45) 100%
    );
  }

  .overlay {
    position: absolute;
    inset: 0;
    z-index: 1;
    pointer-events: none;
  }

  .fallback {
    position: absolute;
    inset: 0;
    z-index: 0;
    background: linear-gradient(160deg,
      #1a2030 0%,
      #15202e 40%,
      #0f1520 100%
    );
  }

  .os-content {
    position: relative;
    z-index: 2;
    width: 100%;
    height: 100%;
    display: flex;
    flex-direction: column;
  }

  /* ─────────────────────────────────────────
     LIQUID GLASS — Global utility class
     Ported from Interacly AI OS DesktopLayer.
     Apply with class="liquid-glass" on any element.
     Requires CSS vars: --c-glass, --c-light, --c-dark,
     --glass-reflex-light, --glass-reflex-dark, --saturation
  ───────────────────────────────────────── */
  :global(.liquid-glass) {
    background-color: color-mix(in srgb, var(--c-glass) 38%, transparent);
    /* Fallback blur */
    backdrop-filter: blur(8px) saturate(var(--saturation));
    -webkit-backdrop-filter: blur(8px) saturate(var(--saturation));
    /* Enhanced SVG displacement filter (if supported) */
    backdrop-filter: blur(8px) saturate(var(--saturation)) url(#switcher);
    -webkit-backdrop-filter: blur(8px) saturate(var(--saturation)) url(#switcher);
    box-shadow:
      inset 0 0 0 1px
        color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 10%), transparent),
      inset 2px 1px 0px -1px
        color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 30%), transparent),
      inset -1.5px -1px 0px -1px
        color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 20%), transparent),
      inset -2px -6px 1px -5px
        color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 40%), transparent),
      inset -1px 2px 3px -1px
        color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 20%), transparent),
      0px 3px 10px 0px
        color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 12%), transparent);
    border: 1px solid rgba(255, 255, 255, 0.12);
  }

  /* Fractal noise overlay — adds subtle texture to glass panels.
     Apply with class="glass-noise" alongside liquid-glass. */
  :global(.glass-noise)  {
    position: relative;
  }
  :global(.glass-noise::after) {
    content: "";
    position: absolute;
    inset: 0;
    border-radius: inherit;
    filter: url(#glass-noise);
    opacity: 0.04;
    pointer-events: none;
    mix-blend-mode: overlay;
    z-index: 0;
  }
</style>
