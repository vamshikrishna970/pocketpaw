---
title: "LLM Providers: Claude, GPT, Gemini & Ollama"
description: "Complete guide to LLM providers and agent backends in PocketPaw. Covers Anthropic, Gemini, Ollama, OpenAI, and OpenAI-compatible endpoints across six agent backends."
section: Core Concepts
ogType: article
keywords: ["llm provider", "model selection", "gemini", "anthropic", "ollama", "openai", "openai-compatible", "model router", "configuration", "claude agent sdk", "openai agents", "google adk", "agent backend"]
...
| Custom endpoint (OpenRouter, vLLM, etc.) | **OpenAI-Compatible** | Varies | OpenAI Agents |
...
**Best for:** OpenAI model users, Ollama users who prefer OpenAI-format APIs.
...
**Best for:** Gemini users who want native Google AI integration with MCP support.
...
- **Copilot SDK** — GitHub Copilot with multi-provider support (Copilot, OpenAI, Azure, Anthropic).
